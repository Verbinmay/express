...
describe.skip("/blogs", () => {
  ...
  it("GET| 404 - FIND BY ID error", async () => {
    await agent.get(info.url.blogs + faker.random.numeric(6)).expect(404);
  });
});
describe.skip("/blogs", () => {
  let blogs: any = [];
...
describe.skip("/blogs", () => {
  ...
  it("DELETE|401  - DELETE BLOG access error", async () => {
    const result = await agent.delete(info.url.blogs + blogs[0].id).expect(401);
  });
...
describe.skip("/posts", () => {
  ...
  it(" GET|200 GET POST BY ID", async () => {
    const result = await agent.get(info.url.posts + post[0].id).expect(200);
...
describe.skip("/posts", () => {
  ...
  it(" PUT|204 - UPDATE POST", async () => {
    ...
    expect(result4.body.shortDescription).toEqual(
      inputPostByIdInfo.shortDescription
    );
...
describe.skip("/posts", () => {
  ...
  it("DELETE|401 -  DELETE POST auth error", async () => {
    const result = await agent.delete(info.url.posts + posts[0].id).expect(401);
  });
  ...
  it("204 DELETE POST ", async () => {
    ...
    const result4 = await agent.get(info.url.posts + posts[0].id).expect(404);
  });
});
describe.skip("/posts", () => {
  let blogs: any = [];
...
describe.skip("/users", () => {
  ...
  it("GET|401- GET USER auth error", async () => {
    const result = await agent.get(info.url.users).expect(401);
  });
});
describe.skip("/users", () => {
  let users: any = [];
...
describe.skip("/users", () => {
  ...
  it("DELETE|401 -DELETE USER auth error", async () => {
    const result = await agent.delete(info.url.users + users[0].id).expect(401);
  });
...
describe.skip("/auth/logout", () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < countOfDevices; i++) {
      const loginUserResponse = await agent
        .post(info.url.auth.login)
  ...
  it("should logout user first device", async () => {
    ...
    const logoutResponse = await agent
      .post(info.url.auth.logout)
      .set("Cookie", userRefreshTokens[0])
      .send({});
