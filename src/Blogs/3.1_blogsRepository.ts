...
export class BlogsRepository  {
    ...
    async createBlog(blog: any) {
        const result: BlogDBModel = await BlogModelClass.create(blog).then(doc => doc.toObject())
        return result
    }
    //UPDATE
    async updateBlog(a: {
        ...
        try {
            ...
            await PostModelClass.updateMany(
                {blogId: a.id},
                {blogName: a.name}
            );
    ...
    async postPostByBlogId(createdPost: any) {
        const result: PostDBModel = await PostModelClass.create(createdPost).then(doc => doc.toObject());
        return result;
    }
