...
export class CommentsService {
    constructor(
        @inject(TYPES.CommentsRepository) protected commentsRepository: CommentsRepository,
        @inject(TYPES.CommentsQRepository) protected commentsQRepository: CommentsQRepository,
    ) {
    }

    //GET
    async findCommentById(a:{commentId:string,userId:string}) {
        const commentFromDB: CommentDBModel | null = await this.commentsQRepository.findCommentById(a.commentId)
